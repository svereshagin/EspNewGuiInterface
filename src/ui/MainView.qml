    ...
    ColumnLayout {
        ...
        RowLayout {
            ...
            Button {
                ...
                onClicked: {
                    if (appStorage) {
                        appStorage.get_controlmodule_info()
                    }
                }
            }
        }
        ...
        GridLayout {
            ...
            Text { text: "Версия ESM:"; font.pixelSize: 13; color: "#666" }
            Text {
                text: appStorage && appStorage.controlModuleInfo ?
                      (appStorage.controlModuleInfo.version || "—") : "—"
                font.pixelSize: 13
                font.family: "Courier"
                color: "#333"
                Layout.fillWidth: true
            ...
            Text { text: "Путь к логам:"; font.pixelSize: 13; color: "#666" }
            Text {
                text: appStorage && appStorage.controlModuleInfo ?
                      (appStorage.controlModuleInfo.logPath || "—") : "—"
                font.pixelSize: 12
                font.family: "Courier"
                color: "#333"
                wrapMode: Text.WordWrap
            ...
            Text { text: "Путь к ControlModule:"; font.pixelSize: 13; color: "#666" }
            Text {
                text: appStorage && appStorage.controlModuleInfo ?
                      (appStorage.controlModuleInfo.appPath || "—") : "—"
                font.pixelSize: 12
                font.family: "Courier"
                color: "#333"
                wrapMode: Text.WordWrap
        ...
        Rectangle {
            ...
            color: "#e3f2fd"
            radius: 4
            visible: appStorage && appStorage.controlModuleInfo &&
                     !appStorage.controlModuleInfo.version

            Text {
                anchors.fill: parent
                anchors.margins: 5
                text: "ℹ️ Информация о Control Module не загружена"
                color: "#1976d2"
                font.pixelSize: 11
                verticalAlignment: Text.AlignVCenter
                horizontalAlignment: Text.AlignHCenter
...
Connections {
    ...
    function onSettingsUpdateCompleted(success, message) {
        console.log("ГИС МТ: обновление настроек", success ? "успешно" : "ошибка", message)
        if (!success) {
            errorBanner.showError(message || "Ошибка обновления настроек ГИС МТ")
        }
    }
}

// ==================== СВЯЗЬ С ЛМ ЧЗ КОНТРОЛЛЕРОМ ====================
Connections {
    target: lmController
    ...
    function onSettingsSaved(success, message) {
        console.log("ЛМ ЧЗ: сохранение настроек", success ? "успешно" : "ошибка", message)
        if (!success) {
            errorBanner.showError(message || "Ошибка сохранения настроек ЛМ ЧЗ")
        }
    }
}

// ==================== СВЯЗЬ С CONTROL MODULE INFO ====================
Connections {
    target: appStorage ? appStorage.controlModuleInfo : null

    function onInfoChanged() {
        console.log("ControlModuleInfo: данные обновлены")
        // Здесь можно обновить UI, если добавите секцию для отображения
    }
}
